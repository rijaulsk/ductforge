...
export default function BoqSheet({
  ...
  /* A zone column only when some extra has a zone, as in the schedule. */
  const extrasZones = project.extras.some((x) => x.zone.trim() !== "");
  const extrasCols = 5 + (extras.priced ? 2 : 0) + (extrasZones ? 1 : 0);
  ...
  return (
    ...
    >
      ...
      {/* THE EXTRAS: dampers, terminals, accessories, custom lines. Their own
        * table, grouped by category, and said out loud to be outside the
        * sheet-metal figures above — a damper is counted here, never added to
        * the duct's area or weight. Rate and value appear only when some line
        * has a rate, like the schedule's own value column. */}
      {on.extras && extras.rows.length > 0 && (
        <section>
          ...
          <table className="mt-[0.5em] w-full border-collapse">
            <thead>
              <tr>
                <th className={TH}>#</th>
                {extrasZones && <th className={TH}>Zone</th>}
            ...
            <tbody>
              {extras.byCategory.map((cat) => (
                <Fragment key={cat.category}>
                  <tr className="break-inside-avoid">
                    <td
                      colSpan={extrasCols}
                      className="pb-[0.2em] pt-[0.7em] text-[0.85em] font-bold uppercase tracking-[0.06em]"
                    >
                      {cat.label}
                    </td>
                  </tr>
                  {extras.rows
                    ...
                    .map(({ extra: x, index, valueMinor }) => (
                      <tr key={x.id} className="break-inside-avoid">
                        ...
                        {extrasZones && <td className={TD}>{x.zone}</td>}
                        <td className={TD}>
                          {x.item}
                          {x.note && <span className="block text-[0.85em]">{x.note}</span>}
                        </td>
                        <td className={`${TD} tabular-nums`}>{describeExtraSize(x, us)}</td>
                        <td className={`${TD} text-right tabular-nums`}>{fmtExact(x.qty, 3)}</td>
                        <td className={TD}>{x.unit}</td>
                        {extras.priced && (
                          <>
                            <td className={`${TD} text-right tabular-nums`}>
                              {x.rate > 0 ? fmtValue(toValueMinor(x.rate)) : ""}
                            </td>
                            <td className={`${TD} text-right tabular-nums`}>
                              {valueMinor ? fmtValue(valueMinor) : ""}
                            </td>
                          </>
                        )}
                      </tr>
            ...
            {extras.priced && (
              <tfoot>
                <tr className="break-inside-avoid">
                  <td className={TF} colSpan={extrasCols - 1}>
