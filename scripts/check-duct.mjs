...
section("12q. the sheet is cut into pages by rule, not by the printer");
{
  const { packPages } = await import("../lib/export/paginate.ts");
  const rows = (n, h = 10) => Array.from({ length: n }, (_, i) => ({ key: `r${i}`, height: h }));
  const table = (n, extra = {}) => ({ kind: "table", key: "t", heading: 0, head: 12, rows: rows(n), total: 14, ...extra });
  const block = (key, height, keepWithNext) => ({ kind: "block", key, height, keepWithNext });

  /* How tall a packed page really is, re-added from the same heights. */
  const heights = (items, pages, continued) => {
    const byKey = new Map(items.map((i) => [i.key, i]));
    return pages.map((p) =>
      p.reduce((s, part) => {
        const it = byKey.get(part.key);
        if (part.kind === "block") return s + it.height;
        const rowH = it.rows.filter((r) => part.rows.includes(r.key)).reduce((a, r) => a + r.height, 0);
        return s + (part.heading ? it.heading : 0) + (part.continued ? continued : 0) + it.head + rowH + (part.total ? it.total : 0);
      }, 0),
    );
  };

  /* Nothing on a page taller than the page, for a spread of real shapes. */
  let worst = 0;
  for (const n of [0, 1, 2, 3, 17, 40, 81, 200]) {
    for (const H of [300, 500, 1000]) {
      const items = [block("head", 120), table(n), block("gauge", 90), block("b", 30, 1), block("b1", 20), block("credit", 40)];
      const pages = packPages(items, H, 16);
      for (const h of heights(items, pages, 16)) if (h > H) worst = Math.max(worst, h - H);
      /* Every row printed exactly once, in order. */
      const printed = pages.flat().filter((p) => p.kind === "table").flatMap((p) => p.rows);
      check(JSON.stringify(printed) === JSON.stringify(rows(n).map((r) => r.key)), `${n} rows on ${H}-high pages: every row once, in order`);
    }
  }
  eq(worst, 0, "no packed page is taller than the paper allows");

  /* A table that runs over repeats its header row, and says it is continued. */
  const long = packPages([table(80)], 300, 16);
  check(long.length > 1, "80 rows do not fit one 300-high page");
  check(long.slice(1).every((p) => p[0].kind === "table" && p[0].continued), "every later page of the table is marked continued");
  check(long[0][0].continued === false, "…and the first is not");

  /* The totals row never sits alone at the top of a page. */
  for (let n = 1; n < 40; n++) {
    const pages = packPages([table(n)], 150, 16);
    const last = pages[pages.length - 1].at(-1);
    check(last.total && last.rows.length > 0, `${n} rows: the totals row keeps a row with it`);
  }

  /* A table does not start at the foot of a page with nothing under its
   * heading: two rows must come with it, or it goes over. */
  const orphan = packPages([block("fill", 250), table(10, { heading: 20 })], 300, 16);
  eq(orphan[0].length, 1, "a heading with no room for two rows moves to the next page");
  check(orphan[1][0].heading === true, "…and arrives there with its heading");

  /* keepWithNext: a heading does not end a page without its first note. */
  const kept = packPages([block("fill", 270), block("h", 20, 1), block("n1", 20)], 300);
  eq(kept[0].length, 1, "a heading that would end the page goes over with its first note");

  /* Something taller than a page gets a page to itself — and packing ends. */
  const huge = packPages([block("a", 50), block("giant", 900), block("c", 50)], 300);
  eq(huge.length, 3, "an oversize block takes a page of its own and the rest follow");
